...
Create a new user  Available userTypes include `User` and `CustomerAdmin` Available userStatus include `Active` and `Inactive`
...
# **deleteUser**
> deleteUser($content_type, $user_id)
...
try {
    $apiInstance->deleteUser($content_type, $user_id);
} catch (Exception $e) {
...
try {
    $apiInstance->listUsers($limit, $offset);
} catch (Exception $e) {
